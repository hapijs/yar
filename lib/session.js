...
internals.Session.prototype.load = function (request, callback) {
    ...
    request[this.settings.session.key][this.settings.session.startKey] = request[this.settings.session.key][this.settings.session.startKey] || 0;
    
    // Check for stale cookie (leftover from server restart)
    
    var fn = 'get';
    if (request[this.settings.session.key][this.settings.session.startKey] != this.settings.session.startTime) {
        var fn = 'delete';
    }
